...
// Ensure the still image fed to Stage 2 perfectly conforms to canvas dimensions without distortion
function conformImageToCanvas(srcPath, dstPath, width, height) {
  const scriptPath = path.join(__dirname, 'resize_to_canvas.py');
  if (fs.existsSync(scriptPath) && fs.existsSync(srcPath)) {
    try {
      require('child_process').execFileSync(
        process.platform === 'win32' ? 'python' : 'python3',
        [scriptPath, srcPath, dstPath, String(width), String(height)],
